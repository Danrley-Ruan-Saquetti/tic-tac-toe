function GameController() {
  ...
  const gameOver = (winner = "", lineProps = []) => {
    ...
    observer.notifyObservers({
      ...
      data: {
        winner: game.winner,
        lineProps: game.lineProps,
        player: game.players.find((player) => player.side == game.winner),
      },
    });
  };
  ...
  function isMate() {
    ...
    const directions = [
      ...
      },
      {
        ...
        direction: { x: 1, y: 1 },
        type: "diagonal",
        typeDirection: "primary",
      },
      {
        position: { x: 0, y: 2 },
        direction: { x: 1, y: -1 },
        type: "diagonal",
        typeDirection: "secondary",
      },
    ];
  ...
  function lance(x, y) {
    ...
    observer.notifyObservers({
      code: "$game/new-lance",
      data: { position: { x, y }, oldSide, currentSide: game.side, player },
    });
  }
